            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <small class="text-muted">Laporan transaksi bulanan</small>
                    </div>
                    <div class="d-flex gap-2">
                        <form method="GET" action="{{ route('admin.transactions.monthly') }}" class="d-flex gap-2">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h3 class="mb-2">Rp {{ number_format($monthlyStats['income'], 0, ',', '.') }}</h3>
                                    <small class="text-success">
                                        <i class="ti ti-arrow-up"></i> {{ Carbon::create($year, $month)->format('F Y') }}
                                    </small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h3 class="mb-2">Rp {{ number_format($monthlyStats['expense'], 0, ',', '.') }}</h3>
                                    <small class="text-danger">
                                        <i class="ti ti-arrow-down"></i> {{ Carbon::create($year, $month)->format('F Y') }}
                                    </small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h3 class="mb-2">Rp {{ number_format($monthlyStats['profit'], 0, ',', '.') }}</h3>
                                    <small class="text-{{ $monthlyStats['profit'] >= 0 ? 'success' : 'danger' }}">
                                        <i class="ti ti-{{ $monthlyStats['profit'] >= 0 ? 'arrow-up' : 'arrow-down' }}"></i> 
                                        {{ $monthlyStats['profit'] >= 0 ? 'Untung' : 'Rugi' }}
                                    </small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h3 class="mb-2">{{ $monthlyStats['count'] }}</h3>
                                    <small class="text-info">
                                        <i class="ti ti-receipt"></i> {{ Carbon::create($year, $month)->format('F Y') }}
                                    </small>
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header">
                    <h5 class="card-title mb-0">Tren Harian</h5>
                    <small class="text-muted">Perbandingan pendapatan dan pengeluaran harian</small>
                </div>
                <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @for($day = 1; $day <= Carbon::create($year, $month)->daysInMonth; $day++)
                                    <?php
                                    $dayData = $dailyBreakdown->get($day, collect());
                                    $income = $dayData->where('type', 'income')->sum('total');
                                    $expense = $dayData->where('type', 'expense')->sum('total');
                                    $net = $income - $expense;
                                    $count = $dayData->sum('count');
                                    ?>
                                    <tr>
                                        <td>{{ $day }} {{ Carbon::create($year, $month)->format('M') }}</td>
                            ...
                            <tfoot>
                                <tr class="fw-bold">
                                    <td>Total</td>
                                    <td class="text-right text-success">Rp {{ number_format($monthlyStats['income'], 0, ',', '.') }}</td>
                                    <td class="text-right text-danger">Rp {{ number_format($monthlyStats['expense'], 0, ',', '.') }}</td>
                                    <td class="text-right {{ $monthlyStats['profit'] >= 0 ? 'text-success' : 'text-danger' }}">Rp {{ number_format($monthlyStats['profit'], 0, ',', '.') }}</td>
                                    <td class="text-right">{{ $monthlyStats['count'] }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div class="card-body">
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            <h5 class="text-muted">Tidak ada transaksi pada {{ Carbon::create($year, $month)->format('F Y') }}</h5>
                            <p class="text-muted">Pilih bulan lain atau tambahkan transaksi baru</p>
